...
As RN-01 a RN-03 delimitam o fluxo implementado na primeira iteração. As RN-04 e RN-05 registram decisões ainda pendentes e não autorizam reoferta ou expiração automática da reserva.
...
**História gigante original:** como Marta, quero que doadores publiquem alimentos excedentes e que ONGs encontrem e reservem essas ofertas para coordenar as doações e evitar conflitos de retirada.

As histórias 1, 3 e 4 decompõem essa história gigante nas fatias demonstráveis **publicar → listar → aceitar**. A história 2 complementa a validação da publicação; as histórias 5, 6 e 7 tratam da evolução posterior à reserva.


**História zero (★)**

**Por que ela:** entrega a menor fatia demonstrável do valor central do produto: tirar a doação da comunicação dispersa e permitir que uma ONG encontre e reserve uma oferta sem que outra organização aceite a mesma doação.
...
| Risco | Prob. | Impacto | Mitigação | Responsável | Prazo |
|---|:---:|:---:|---|---|---|
| Doador abandonar um cadastro demorado. | Média | Alto | Manter três campos e testar com três doadores. | Marta e equipe | Antes do piloto. |
| Doação ser aceita e não retirada no prazo. | Alta | Alto | Acompanhar os casos manualmente e validar a regra de tratamento, sem reoferta automática. | Marta e ONGs | Antes do piloto. |

## Hipótese e experimento

**Hipótese falsificável:** ao fim de um piloto de 14 dias em um bairro, pelo menos 70% das doações publicadas no período serão retiradas antes da validade usando o fluxo centralizado.

**Experimento:** operar com ao menos três doadores e duas ONGs. Como o sistema ainda não registra a retirada, Marta acompanhará em uma planilha a publicação, a validade, o aceite e a confirmação da retirada. Ao final, será calculado `doações retiradas antes da validade / doações publicadas × 100`. Doações sem retirada confirmada permanecem no denominador, mas não entram no numerador. Resultado inferior a 70% invalida a hipótese; se nenhuma doação for publicada, o resultado será “sem dados”.
